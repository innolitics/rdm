...
def test_single_issue(tmp_repo):
	...
	assert subprocess.check_output(['git', 'show', '-s', '--format=%B'], encoding='utf-8') == "Fix some issue\n\nIssue #10\n\n"
...
def test_no_issue_number(tmp_repo):
	...
	assert "Aborting commit. Your commit message is missing either a Github Issue ('Issue #131') or 'Merge'" in str(Error.value)